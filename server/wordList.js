...
async function treatText(text){
    const list = await text.split('\n')
    return list.map(e => e.replace('\r', "")).map(e => removeDiacritics(e)).map(e => e.toUpperCase())
}

async function getRelevantWords(arr){
    
    const result = await arr.filter(checkLength)
...
async function findPossibleWords(obj){
    ...
    const possibleWords = correctFilter.map(e =>{
        ...
        for(let i=0; i<obj.wrong.length; i++){
            if(e.includes(obj.wrong[i])){
                return null
            }
        }
        return e
